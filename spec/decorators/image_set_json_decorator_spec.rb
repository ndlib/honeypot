...
describe ImageSetJsonDecorator do
  ...
  describe '#id' do
    it "returns the href to the api for the image" do
      expect(subject.send(:id)).to eq("http://test.host/api/images/path/to/image.jpg")
    end
  end
  ...
  describe '#derivatives' do
    it "returns decorated derivatives" do
      ...
      expect(DerivativeJsonDecorator).to receive(:new).with(derivative).and_return('decorated')
